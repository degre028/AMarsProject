...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "aamarsproject" because\n';
  ...
  $wnd.__gwt_activeModules['aamarsproject'] = {
    'moduleName' : 'aamarsproject',
    'bindings' : function() {
      return {};
    }
  };
 ...
 * "/aamarsproject.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['aamarsproject'].superdevmode = true;

  var expectedSuffix = '/aamarsproject.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + aamarsproject + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:aamarsproject';
  var devModeSessionKey = '__gwtDevModeSession:aamarsproject';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/aamarsproject/aamarsproject.nocache.js';
  ...
  $wnd.__gwt_activeModules['aamarsproject'].canRedirect = true;
  $wnd.__gwt_activeModules['aamarsproject'].superdevmode = true;
  ...
  css = (moduleIdx == 1 ? css : '') +
    ".gwt-DevModeModule-" + moduleIdx + ".gwt-DevModeCompiling div:before{" +
      "content:'COMPILING aamarsproject';" +
      "font-size:24px;" +
      "color:#d2d9ee;" +
    "}";
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/aamarsproject?user.agent=' + ua + '&_callback=' + callback;
